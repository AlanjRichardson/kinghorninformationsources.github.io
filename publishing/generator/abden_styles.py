...
def find_font(candidates, required_terms):
    ...
    for font in fonts:
        ...
        if all(term in test for term in terms):
            return font
    related = [font for font in fonts if "libert" in font.casefold() or "biolin" in font.casefold()]
    raise RuntimeError(
...
def discover_fonts():
    return {
        "serif_regular": find_font(["Linux Libertine O Regular", "Linux Libertine O"], ["linux", "libertine", "regular"]),
        "serif_italic": find_font(["Linux Libertine O Italic"], ["linux", "libertine", "italic"]),
        "serif_bold": find_font(["Linux Libertine O Bold"], ["linux", "libertine", "bold"]),
        "sans_regular": find_font(["Linux Biolinum O Regular", "Linux Biolinum O"], ["linux", "biolinum", "regular"]),
        "sans_bold": find_font(["Linux Biolinum O Bold"], ["linux", "biolinum", "bold"]),
    }
...
def create_character_styles(fonts):
    ...
    definitions = [
        ...
        ("Abden Title Series Character", fonts["sans_regular"], t["title_series_size"]),
        ("Abden Title Main Character", fonts["sans_bold"], t["title_main_size"]),
        ("Abden Title Volume Character", fonts["sans_bold"], t["title_volume_size"]),
    ...
    for name, font, size in definitions:
        if name not in existing:
            scribus.createCharStyle(name=name, font=font, fontsize=size, fillcolor="Black", language="en_GB")


def create_paragraph_styles():
    t = TYPOGRAPHY
    mm5, mm7, mm10 = 14.173228, 19.842520, 28.346457
    definitions = [
        ...
        dict(name="Abden Title Series", linespacingmode=0, linespacing=t["title_series_leading"], alignment=1, firstindent=0.0, gapbefore=0.0, gapafter=0.0, charstyle="Abden Title Series Character"),
        dict(name="Abden Title Main", linespacingmode=0, linespacing=t["title_main_leading"], alignment=1, firstindent=0.0, gapbefore=0.0, gapafter=0.0, charstyle="Abden Title Main Character"),
        dict(name="Abden Title Volume", linespacingmode=0, linespacing=t["title_volume_leading"], alignment=1, firstindent=0.0, gapbefore=0.0, gapafter=0.0, charstyle="Abden Title Volume Character"),
